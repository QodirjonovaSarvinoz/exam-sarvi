let kategoriya = document.getElementById("kategoriya")
let sliceKategoriyaProduct = products.slice(products.length -28, products.length -22)
sliceKategoriyaProduct.map((el) =>{
    kategoriya.innerHTML += `
     <div
                class="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col">
                <div class="relative">
                    <img src="${el.images[0]}" alt="product" class="w-full h-[140px] sm:h-[202px] rounded-2xl">
                    <img src="./images/heard.png" alt="heart" class="absolute top-3 right-3 w-6 h-6 cursor-pointer">
                </div>

          <p class="text-[17px] font-semibold text-gray-900 leading-none mb-1 mt-5 ml-2">${el.price}₽</p>
          <p class="text-sm text-gray-600 mb-2 ml-2">
            ${el.description}
          </p>

          <div class="flex items-center gap-[2px] mb-3 ml-2">
            <img src="./images/star.png" alt="star" class="w-4 h-4" />
            <img src="./images/star.png" alt="star" class="w-4 h-4" />
            <img src="./images/star (1).png" alt="star" class="w-4 h-4" />
            <img src="./images/star (1).png" alt="star" class="w-4 h-4" />
            <img src="./images/star (1).png" alt="star" class="w-4 h-4" />
          </div>

           <a href="./korzina.html"><button
                            class="mt-auto border border-green-600 text-green-600 font-semibold rounded-lg py-2 w-full transition-all duration-300 hover:bg-orange-500 hover:border-orange-500 hover:text-white">
                            В корзину
                        </button></a>
        </div>`
})